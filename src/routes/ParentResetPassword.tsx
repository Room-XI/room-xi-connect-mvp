...
export default function ParentResetPassword() {
  ...
  const validateToken = async (token: string) => {
    setValidating(true);
    try {
      const { data, error: apiError } = await api.parentAuth.validateResetToken(token);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: apiError } = await api.parentAuth.resetPassword(token!, password);
      ...
      <div className="min-h-screen bg-gradient-cosmic flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-textSecondaryLight mb-6">
            {tokenError || 'This password reset link has expired or is invalid.'}
          </p>
      ...
      >
        <div className="text-center mb-6">
          ...
          <p className="text-textSecondaryLight mt-2">
            Create a new password for your parent portal account.
          </p>
        </div>
        ...
        <div className="mt-4 text-center">
          <Link to="/parent/login" className="text-sm text-textSecondaryLight hover:text-deepSage flex items-center justify-center gap-1">
