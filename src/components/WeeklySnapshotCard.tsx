...
import { Calendar, X, TrendingUp, TrendingDown, Minus } from 'lucide-react';
import { DateTime } from 'luxon';
import { useMoodGradient } from '@/hooks/useMoodGradient';
import { getMoodByScore } from '@/lib/moodConfig';
import api from '@/lib/api';
import type { MoodSummary } from '@/hooks/useMoodGradient';
...
export default function WeeklySnapshotCard({ onClose }: WeeklySnapshotCardProps) {
  const { summary: thisWeek } = useMoodGradient(7);
  const [lastWeek, setLastWeek] = useState<MoodSummary | null>(null);
  ...
  const totalCheckIns = thisWeek.daysWithData; // Days with at least 1 check-in
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Check-in count with comparison */}
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-baseline gap-2">
                ...
                {lastWeek && lastWeek.daysWithData !== totalCheckIns && (
                  <div className="flex items-center gap-1 text-sm">
                    {totalCheckIns > lastWeek.daysWithData ? (
                      <><TrendingUp className="w-4 h-4 text-green-600" /><span className="text-green-600 font-medium">+{totalCheckIns - lastWeek.daysWithData}</span></>
                    ) : totalCheckIns < lastWeek.daysWithData ? (
                      <><TrendingDown className="w-4 h-4 text-orange-600" /><span className="text-orange-600 font-medium">{totalCheckIns - lastWeek.daysWithData}</span></>
        ...
        {totalCheckIns > 0 ? (
          <div className="mb-6">
            ...
            <div className="h-12 rounded-lg overflow-hidden flex">
              {Object.entries(thisWeek.ratios).map(([label, ratio]) => {
                ...
                if (percentage < 1) return null; // Don't show tiny slices
            ...
            <div className="grid grid-cols-2 gap-2 mt-3">
              {Object.entries(thisWeek.ratios).filter(([,ratio]) => ratio > 0.01).map(([label, ratio]) => {
                const mood = [1, 2, 3, 4, 5, 6]
                  .map(score => getMoodByScore(score))
                  .find(m => m.label === label);
